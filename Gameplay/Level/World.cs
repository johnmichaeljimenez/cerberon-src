...
public class World : IDisposable //aka Level loader
{
	...
	public void Init(GameplayState gameplayState)
	{
		...
		gameplayState.GetManager<GameplayEventManager>().FireTrigger(EventTypes.StartGame);
	}
		...
		foreach (var i in Sprites)
		{
			...
			if (i.Key == 0)
			{
				...
				foreach (var j in Entities)
				{
					...
					var shader = RenderingManager.BeginEntityShader();
					shader.SetValue("maskAmount", 1.0f);
				...
				foreach (var j in Entities)
				{
					...
					var shader = RenderingManager.BeginEntityShader();
					shader.SetValue("maskAmount", j.SpriteMaskAmount);
					j.Draw();
					Raylib.EndShaderMode();
				}
